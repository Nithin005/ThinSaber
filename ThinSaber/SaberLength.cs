...
namespace ThinSaber
{
    public class SaberLength : MonoBehaviour
    {
        ...
        private bool EnableHitboxScaling = false;
        private float LengthMultiplier = 1;
        private bool IsMultiplayerEnv = false;
        private float SaberWidth = Config.Instance.SaberWidth;
...
#pragma warning restore IDE0051 // Used by MonoBehaviour
        ...
        private void Load()
        {
            ...
            for (int i = 0; i < SceneManager.sceneCount; i++)
            {
                ...
                if (SceneManager.GetSceneAt(i).name == "MultiplayerGameplay")
                {
                    ...
                    Plugin.Log?.Info("Multiplayer environment detected, disabling hitbox scaling and cosmetic length scaling.");
                }
                ...
                {
                    EnableHitboxScaling = false;
                    Plugin.Log?.Info("Campaign environment detected, disabling hitbox scaling.");
                }
            }
            ...
            if (EnableHitboxScaling)
            {
                ...
                Plugin.Log?.Info("ScoreSubmission has been disabled.");
            }
        ...
        private IEnumerator ApplyGameCoreModifications()
        {
            ...
            Plugin.Log?.Info("sabers count: "+sabers.Length);
            foreach (Saber saber in sabers)
            {
                ...
                if (IsMultiplayerEnv || IsSoloGameplaySaber(saber.gameObject))
                {
                    ...
                    RescaleSaber(saber.gameObject, LengthMultiplier, SaberWidth);
                    Plugin.Log?.Info("Saber Scaled");
        ...
        private void OnDestroy()
        {
            ...
            foreach (Saber saber in sabers)
            {
                RescaleSaber(saber.gameObject, 1 / LengthMultiplier, 1 / SaberWidth);
            }
        }
